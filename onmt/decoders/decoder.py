...
class RNNDecoderBase(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        if copy_attn and not reuse_copy_attn:
            ...
            # self.copy_attn = Gated_Attention(
            #     hidden_size, attn_type=copy_attn_type, attn_func=attn_func
            # )
        else:
            self.copy_attn = None
    ...
    #             **kwargs):
    def forward(self, tgt, h_s, auxi_hs, memory_lengths=None, step=None,
                **kwargs):
        """
        Args:
            tgt (LongTensor): sequences of padded tokens
                 ``(tgt_len, batch, nfeats)``.
            h_s (FloatTensor): vectors from the encoder
                 ``(src_len, batch, hidden)``.
            auxi_hs(FloatTensor): vectors from our auxiliary network.
        ...
        dec_state, dec_outs, attns = self._run_forward_pass(
            tgt, h_s, auxi_hs,memory_lengths=memory_lengths)
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, h_s, auxi_hs,memory_lengths=None):
        """
        Args:
            tgt (LongTensor): a sequence of input tokens tensors
                ``(len, batch, nfeats)``.
            h_s (FloatTensor): output(tensor sequence) from the
        ...
        Returns:
            ...
            * dec_outs: an array of output hidden state(h_hat) of every time step from the decoder.
        ...
        emb = self.embeddings(tgt) # put the target examples into the embedding layers.
        ...
        for emb_t in emb.split(1):
            # input_feed--h_hat every previous time
            decoder_input = torch.cat([emb_t.squeeze(0), input_feed], 1) # 将h_hat与input
            rnn_output, dec_state = self.rnn(decoder_input, dec_state)
            # rnn_output--每次decoder的current hidden state
            # dec_state--(h,c)
            if self.attentional:
                decoder_output, p_attn = self.attn(
                    rnn_output,
                    h_s.transpose(0, 1), # 转置=>(batch,src_len,hidden_size)
                    auxi_hs.transpose(0,1),
                    memory_lengths=memory_lengths)
                attns["std"].append(p_attn) # 把每次的attn也存了下来
            else:
                decoder_output = rnn_output

            # decoder_output--h_hat, p_attn = alpha
            ...
            input_feed = decoder_output # update the next time input
            ...
            if self.copy_attn is not None:
                _, copy_attn = self.copy_attn(
                    decoder_output, h_s.transpose(0, 1))
...
'''I don't need the following code.'''
class StdRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, auxi_hs=None,memory_lengths=None):
        """
        ...
        if isinstance(self.rnn, nn.GRU):
            rnn_output, dec_state = self.rnn(emb, self.state["hidden"][0]) # LSTM
        else:
